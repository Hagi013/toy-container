...
fn main() -> Result<()> {
    ...
    let cb = Box::new(|| {
        ...
        if (*MOUNT_DIR.read().unwrap()).is_some() {
            ...
            spaws_sh_tracable(&*COMMAND.read().unwrap(), self_pid);
        }
        loop {
    ...
    loop {
        ...
        println!("first pid: {:?}", pid);
        let pid_num: i64 = ptrace::get_event(pid)?;
        println!("first pid: {:?}, get_event: {:?}", pid, pid_num);

        if pid_num != 0 && pid_num != parent_pid.as_raw() as i64 {
            let child_pid = Pid::from_raw(pid_num as libc::pid_t);
            let child_status = ptrace::wait_pid(child_pid).unwrap();
            println!("child_status, pid: {:?}, child_status: {:?}", child_pid, child_status);
            check_signal(child_status).unwrap();
            let mut child_regs: user_regs_struct = ptrace::getregs(pid).unwrap();
            prev_orig_rax_by_pid.insert(child_pid, child_regs.orig_rax);
            handle_trap(pid, child_status);
        }
        ...
        if emulate_flag {
            ...
            handle_trap(pid, end_status).unwrap();
            ptrace::syscall(pid);
        }
        emulate_flag = false;
    }
...
pub fn spaws_sh_tracable(command: &str, pid: Pid) {
    // let child = unsafe {
    //     Command::new("/bin/sh")
    //         .pre_exec(|| {
    ...
    //         .unwrap()
    // };
    // let child: std::process::Child = match ptrace::wait_pid(Pid::from_raw(child.id() as libc::pid_t)) {
    //     Ok(WaitStatus::Stopped(pid, signal::SIGTRAP)) => {
    //         println!("spaws_sh_tracable, pid: {:?}", pid);
    //
    //         child
    //     },
    //     Err(e) => {
    //         println!("Error in spaws_sh_tracable!!!!!:{:?}", e);
    //         panic!("Not Stopped Child Process...{:?}", pid)
    //     },
    //     e => {
    //         println!("Error2 in spaws_sh_tracable:{:?}", e);
    //         panic!("Not Stopped Child Process...{:?}", pid)
    //     }
    // };
    println!("spaws_sh_tracable!!!!!!!!!!");
    let child = unsafe { Command::new("/bin/sh")
            .pre_exec(|| {
                ptrace::traceme().map_err(|e| {
                    println!("traceme error: {:?}", e);
    ...
    println!("aaaaaaaaaaaaaaaaa!!!!!!!!!!!bbbbbbbbbb");
    let output = child
        .wait_with_output()
        .unwrap();
    println!("spaws_sh_tracable, pid: {:?}, Status: {:?}, command: {:?}", getpid(), output.status, command);
    let stdout = output.stdout;
    let lines = String::from_utf8(stdout).unwrap();
    let split_by_line: Vec<&str> = lines.split('\n').collect();
    for &line in split_by_line.iter() {
...
fn handle_trap(org_pid: Pid, status: WaitStatus) -> Result<Option<bool>> {
    match status {
        WaitStatus::PtraceEvent(pid, sig, int) => {
            match sig {
                signal::SIGTRAP => {
                    let siginfo: libc::siginfo_t = ptrace::get_siginfo(pid).unwrap();
                    println!("siginfo: {:?}, si_value: {:?}, si_addr: {:?}", siginfo, unsafe { siginfo.si_value() }, unsafe { siginfo.si_addr() });
                    let pid_num: i64 = ptrace::get_event(pid)?;
                    println!("org_pid: {:?}, pid: {:?}, get_event: {:?}", org_pid, pid, pid_num);
                    // ptrace::syscall_step(pid);
                    // ptrace::syscall_step(Pid::from_raw(pid_num as libc::pid_t));
                    ptrace::syscall(Pid::from_raw(pid_num as libc::pid_t));
                    ptrace::syscall(org_pid);
                    let org_status = ptrace::wait_pid(pid).unwrap();
                    println!("org_status: {:?}", org_status);
                    return Ok(Some(true))
                },
                _ => {
        ...
        WaitStatus::PtraceSyscall(pid) => {
            if org_pid == pid { return Ok(Some(false)); }
            let mut child_regs: user_regs_struct = ptrace::getregs(pid).unwrap();
            println!("spawn enter!, pid: {:?}, orig_rax: {:?}, rsi: {:?}, rdx: {:?}, rdi: {:?}, rax: {:?}", pid, child_regs.orig_rax, child_regs.rsi, child_regs.rdx, child_regs.rdi, child_regs.rax);
            ptrace::syscall(pid);
            let status = ptrace::wait_pid(pid).unwrap();
            println!("spawn status: {:?}", status);
            // child_regs.rax = 0;
            let mut child_regs: user_regs_struct = ptrace::getregs(pid).unwrap();
            println!("spawn exit!!, pid: {:?}, orig_rax: {:?}, rsi: {:?}, rdx: {:?}, rdi: {:?}, rax: {:?}", pid, child_regs.orig_rax, child_regs.rsi, child_regs.rdx, child_regs.rdi, child_regs.rax);
            ptrace::setregs(pid, child_regs);
            ptrace::syscall(pid);
            return Ok(Some(false))
        },
        _ => {
